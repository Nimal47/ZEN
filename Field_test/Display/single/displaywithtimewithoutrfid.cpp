...
void displayTask(void *parameter) {
  ...
  for (;;) {
    ...
    // RULE 1: 0 to 3 seconds -> Show the Time
    if (timeSinceMotion < pdMS_TO_TICKS(3000)) {
      struct tm timeinfo;
      if (getLocalTime(&timeinfo)) {
        // Only redraw if the state changed OR the second changed (prevents flicker)
        if (currentState != STATE_TIME || timeinfo.tm_sec != lastSecond) {
          if (currentState != STATE_TIME) tft.fillScreen(ST77XX_BLACK); // Clear screen once
          
          tft.setCursor(15, 60);
          // Drawing text with a background color prevents it from overlapping itself
    ...
    // RULE 2: 3 to 10 seconds -> Show Image
