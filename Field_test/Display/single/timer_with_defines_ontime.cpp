...
// --- SYSTEM TIMING CONSTANTS (Modify these easily!) ---
#define SCREEN_AWAKE_MS       10000  // Time screen stays fully awake (10 sec)
#define SCREEN_SLEEP_WARN_MS  12000  // Time screen turns off after (12 sec)
#define TIMER_AUTOSTART_SEC   5      // Seconds before the timer auto-starts
#define TIME_UP_DISPLAY_MS    5000   // How long "TIME UP" shows before reverting to default (5 sec)
#define RFID_TIMEOUT_MS       2000   // How long phone can be lifted before timer stops (2 sec)
...
// --- TIMER VARIABLES ---
volatile int timerMinutes = 1;  // Now starts at 1 minute!
volatile TickType_t lastTimerInteractionTick = 0; 
volatile bool isTimerRunning = false;             
volatile TickType_t timerStartTick = 0;           
volatile TickType_t timeUpStartTick = 0; // Tracks when "TIME UP" appeared
...
void rfidTask(void *parameter) {
  ...
  for (;;) {
    ...
    if (status == MFRC522::STATUS_OK) {
      ...
      // Uses the new RFID_TIMEOUT_MS constant
...
void displayTask(void *parameter) {
  ...
  for (;;) {
    ...
    // RULE 1: AWAKE (Uses the new SCREEN_AWAKE_MS constant)
    if (timeSinceMotion < pdMS_TO_TICKS(SCREEN_AWAKE_MS)) {
      ...
      // --- MODE 1: TIMER INTERFACE ---
        ...
        // PHASE A: WAITING TO START
        if (!isTimerRunning) {
          // Uses the new TIMER_AUTOSTART_SEC constant
        ...
        // PHASE B: TIMER IS ACTIVELY RUNNING
          ...
          // PHASE C: TIME IS UP
            ...
            // Actively feed the watchdog so the screen stays awake to show "TIME UP!"
            ...
            if (lastDrawnSecond != 0 || justWokeUp) {
              ...
              // Record the exact moment "TIME UP" was drawn on the screen
              if (lastDrawnSecond != 0) {
                 timeUpStartTick = currentTick; 
              }
              lastDrawnSecond = 0; 
            }

            // AUTO-REVERT LOGIC: Switch back to default mode after 5 seconds
            if (currentTick - timeUpStartTick > pdMS_TO_TICKS(TIME_UP_DISPLAY_MS)) {
               currentMode = MODE_DEFAULT;     // Go back to the image
               isTimerRunning = false;         // Reset the timer state
               timerMinutes = 1;               // Reset to the 1-minute default
               lastMotionTick = currentTick;   // Ensure the screen stays awake to show the change
               Serial.println("🔄 Time Up finished. Reverting to Default Mode.");
            }
          }
        }
      }
    ...
    // RULE 2: SLEEPING WARNING (Uses new SCREEN_SLEEP_WARN_MS constant)
