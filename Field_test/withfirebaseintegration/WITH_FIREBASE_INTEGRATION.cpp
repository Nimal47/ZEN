...
#define FIREBASE_HOST "adis-d59e0-default-rtdb.europe-west1.firebasedatabase.app"
#define FIREBASE_AUTH "xWcJvIEz580mz5AVZwynHRLOhiRUqUDHX0MsOsXg"
        ...
        // --- MODE 3: REMINDER ---
          ...
          if (currentState != STATE_AWAKE_REMINDER) {
            if (xSemaphoreTake(spiMutex, portMAX_DELAY)) {
              ...
              tft.setTextSize(1);
              int yOffset = 50; // Starting height for the list
              ...
              if (reminderList.empty()) {
                  ...
                  // Loop through all reminders and draw them
                  for (size_t i = 0; i < reminderList.size(); i++) {
                      if (yOffset > 140) break; // Stop drawing if we run out of screen space!
                      ...
                      // Limit text to 13 characters so it fits neatly on the 128px screen
                      tft.print(reminderList[i].text.substring(0, 13)); 
                      
                      yOffset += 20; // Move down 20 pixels for the next line
                  }
              }
              xSemaphoreGive(spiMutex);
            }
